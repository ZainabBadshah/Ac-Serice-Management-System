...
Class Master extends DBConnection {
	...
	function save_mechanic(){
		...
		$check = $this->conn->query("SELECT * FROM `technician_list` where `name` = '{$name}' ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
		if($this->capture_err())
			return $this->capture_err();
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "Techncian already exist.";
			return json_encode($resp);
			exit;
		}
		if(empty($id)){
			$sql = "INSERT INTO `technician_list` set {$data} ";
			$save = $this->conn->query($sql);
		}else{
			$sql = "UPDATE `technician_list` set {$data} where id = '{$id}' ";
			$save = $this->conn->query($sql);
		}
		if($save){
			$resp['status'] = 'success';
			if(empty($id))
				$this->settings->set_flashdata('success',"New Technician successfully saved.");
			else
				$this->settings->set_flashdata('success',"Technician successfully updated.");
		}else{
			$resp['status'] = 'failed';
			$resp['err'] = $this->conn->error."[{$sql}]";
		}
	...
	function delete_mechanic(){
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `technician_list` where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Technician successfully deleted.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
