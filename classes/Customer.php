...
Class Customer extends DBConnection {
	...
	public function save_users(){
		extract($_POST);
		$data = '';
		$chk = $this->conn->query("SELECT * FROM `customer` where username ='{$username}' ".($id>0? " and id!= '{$id}' " : ""))->num_rows;
		if($chk > 0){
			return 3;
			exit;
		}
		...
		if(empty($id)){
			...
			if($qry){
				...
				return "UPDATE customer set $data where id = {$id}";
			}
	...
	public function delete_users(){
		extract($_POST);
		$avatar = $this->conn->query("SELECT avatar FROM customer where id = '{$id}'")->fetch_array()['avatar'];
...
switch ($action) {
	case 'save':
		echo $customer->save_users();
	break;
	case 'fsave':
		echo $customer->save_fusers();
	break;
	case 'ssave':
		echo $customer->save_susers();
	break;
	case 'delete':
		echo $customer->delete_users();
	break;
	default:
		// echo $sysset->index();
		break;
